...
public:
	...
	//后置++，用占位符int区分前置和后置递增，double,float不行
...
父类中虚函数对应一个vfptr（虚函数指针，占4个字节），指向vftable（虚函数表），并继承到子类中。

子类中重写了vfptr对应的内容vftable，使得vfptr 指向&cat（对应视频136）
